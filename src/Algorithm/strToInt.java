...
public class strToInt {
    ...
    public static int strToInt(String str) {
        int result = 0;
        int flag = -1;
        int numFlag = 0; //第一次遇到数字
        int zeroFlag = 0;
        Stack<Integer> integers = new Stack<>();
        for (char temp : str.toCharArray()) {
            if (numFlag == 1 && (temp > '9' || temp < '0')) {
                break;
            }
            if (temp == ' ') {
                continue;
                ...
                numFlag = 1;
                if (zeroFlag == 0 && temp == '0') {
                    continue;
                }
                integers.push(temp - 48);
                zeroFlag = 1;
            } else {
                break;
            }
        ...
        if (numFlag == 1) {
            ...
            while (!integers.isEmpty()) {
                int x = integers.pop();
                temp = temp + (long)(x * n);
                if (temp > Integer.MAX_VALUE || n > Integer.MAX_VALUE) {
                    if (flag == 1) {
